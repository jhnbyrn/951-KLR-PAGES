...
In the above section, if 4D gets to 16 or higher, then we consider the correction to have run it's course and we clear some flags and bail out of this fuel correction section. The flag 22h.6 is the highest priority timing speed regulator, and it's set when 21h.5 and 21h.6 are set (these are the flags that control this fuel correction condition). All that happens in a [different routine](dme_fuel_cut_code.md) where the fuel cut off logic is implemented. 

So in summary, we modulate the fuel pulse by the map cell indicated by 4D until 4D reaches 16. Then we clear the ignition speed limiter 22h.6. We also clear that limiter if the map value is 64 (i.e. 1). In these cases we also reset 34h to 1 which will enable the timing speed change to take effect immediately. In plain terms: ignition timing rate of change is temporarily limited while we're making this fuel correction. 
