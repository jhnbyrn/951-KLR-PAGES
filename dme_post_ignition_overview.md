...
* if 21h.7 is set, add the acceleration enrichment value 4C to the main injector pulse
* add the voltage-based injector latency (x5) to the pulse
* complement the final pulse value
* clear the timer0 registers, load the value, and turn the injectors on (via p1.0)
* set 21h.2 as an *injection on* flag (we used this earlier for the extra injection event)
* tidy up before returning from the routine

The injector latency value is loaded from Map 26 into 54h in __040D__, which is called after the main fuel enrichments routine (see the end of the [code walkthrough](dme_fuel_enrichments_code_walkthrough.md) of that section). 
