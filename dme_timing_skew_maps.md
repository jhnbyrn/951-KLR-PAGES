...
In this article we'll examine a peculiar map used during [ignition timing calculation](dme_ignition_timing_calculation_overview.md) in both the NA and the Turbo 944s. It's Map 72 (location __12C0__) and its a mostly linear function that's used to *skew* the main timing maps around a certain rpm - that is, timing is reduced below that rpm and increased above it, by an amount that's proportional to the change in rpm. 
...
In the main timing calculation routine (at __1C24__) our skew map gets added to the final calculation after the idle, part throttle or WOT map lookup (this addition happens at __1C8F__).
...
But things get a little more interesting when we look at the turbo version:
