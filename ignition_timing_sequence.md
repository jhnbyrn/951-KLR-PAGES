...
![](images/ignition_timing/spark_cyl_A_1.png)

We do this at 00B8 and load that value into 2B. 

Next we calculate the half-tooth count to the next TDC for use in the future. This (among other things) happens in the routine at 021D.  We start with the half-tooth count for 180 degrees (132), and we add our total spark advance count (31h + 57h). We don't take dwell into account here because this next-TDC value will later be used relative to the ignition event. 

This can be a bit confusing, but the next TDC value that we used was calculated after the *previous* spark event, at cyl B - let's visualize it like this:


![](images/ignition_timing/next_TDC_calculation_from_B_1.png)
