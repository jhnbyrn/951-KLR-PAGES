...
0xe8d cpl  f0
0xe8e cpl  a			;a = |52h - 53h|
```
If b3 of the scheduler value is set, we jump directly to the CV final output routine. 

Otherwise, we calculate the difference between current boost 52h and filtered target boost 53, convert it to an absolute value and track the sign with f0. The default for f0 here is 1 (from DDA), so now we set f0=0 if current boost is higher than the target.  
...
0xecf mov  @r0,a
```
This limits 61h to between 68 and 187, that is -60 to +59 around our pseudo-zero value of 128. If we're at either rail, we inc or dec 67h accordingly. 

That covers the integral terms 61h and 67h (although 67h is adjusted separately, back towards its centre value 128 whenever the final cycling valve PWM output is saturated). 
...
What about 60h? Suppose that we have an overboost situation, and 60h and 61h are both somewhat negative. Let's say, -60 each. For 60h, that means a literal value of 196 and for 61h, a literal value of 68. Added together, we get an overflow, and a final value of 8. That doesn't look negative, but when we add 62h (which is generally 128 in the case of overboost) we'll have 136, which means -20 in the usual signed 2s complement form. So all is well. 
...
This means that if 61h is saturated negative, and 60h is growing in the negative direction, when it exceeds -68 (relative), the overall correction value will flip from -128 to +127 instantly. 
